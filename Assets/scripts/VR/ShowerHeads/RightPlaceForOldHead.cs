    ...
    private void OnTriggerEnter(Collider col)
    {
        if (MiniGameManager.isHeadShowerGameRunning)
        {
            if (col.gameObject.tag == "OldShowerHead")//the old one
            {
                ...
                EventBus.TriggerEvent(this, new NarrativeEvent.TextToSpeechNarratorEvent(false,"Neat, now you just have to pick up the new shower head and screw it in clockwise."));
